...
inline int cr_resolve_wave(GameState* state, int8_t* next_tokens, int8_t* next_owners, int8_t* pressure) {
    int exploded = 0;
...
inline int cr_step(GameState* state, int action_idx, int8_t player_id) {
    ...
    while (cr_resolve_wave(state, next_tokens, next_owners, pressure)) {
