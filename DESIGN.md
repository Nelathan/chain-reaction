...
- **Self-play reward convention:** each completed game produces two PufferLib log entries — winner (+1) and loser (-1) — so both sides receive explicit zero-sum terminal reward signals. Without the loser entry, the losing player's final move has reward 0 (game ongoing) and the critic must infer defeat entirely through value bootstrap. Actual skill is tracked via player-1 win rate, not mean episode return.
